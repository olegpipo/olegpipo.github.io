...
1. Use a WHOIS lookup (whois.domaintools.com or similar) to find the domain's registrar and hosting provider.
2. Contact the registrar's abuse department (usually abuse@registrar.com).
3. Contact the hosting provider's abuse department.
...
In the United States, forward [smishing](/phishing/smishing/) messages to **7726** (which spells SPAM). This service is supported by major carriers (AT&T, Verizon, T-Mobile) and helps them identify and block spam and phishing text messages.
...
If the call involved [deepfake voice cloning](/phishing/deepfake-voice-scams/) -- someone impersonating a family member or known person using a cloned voice -- report it to:
...
[QR code scams](/phishing/qr-code-scams/) require a multi-step reporting approach:

1. **Report the URL** to Google Safe Browsing and your browser (see website reporting above).
2. **Report the physical QR code** to the location where you found it -- notify the business, parking authority, or property manager so they can remove the fraudulent sticker.
3. **Report to local law enforcement** if the QR code is on public infrastructure (parking meters, transit stations).
...
File a report at **ic3.gov** for any internet-related crime, including phishing, [business email compromise](/phishing/business-email-compromise/), [romance scams](/phishing/romance-scams/), and [tech support scams](/phishing/tech-support-scams/). IC3 reports are analyzed and distributed to federal, state, and local law enforcement agencies.
...
1. **Change compromised passwords** using a [password manager](/password-managers/) to generate [strong, unique replacements](/password-security/). If you are on macOS or iOS, [PanicVault](https://apps.apple.com/app/id6759188575) makes it easy to generate and store strong passwords in the open KeePass KDBX format.
...
5. **Consider a credit freeze** if personal financial information was exposed.
...
1. Report it to your email provider (one click in most email apps).
2. Forward it to reportphishing@apwg.org (one forward).
3. Delete it.
...
[Training your family to report phishing](/phishing/train-family/) multiplies this impact. If every member of your household reports the phishing messages they receive, you are feeding more data into the systems that protect everyone.
