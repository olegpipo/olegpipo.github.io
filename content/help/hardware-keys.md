...
For an iCloud vault, a save that needs a fresh answer from your key is held until it gets one, and the banner says **Changes not saved to the vault file — YubiKey needed.** It says the same when a sync finds the vault waiting for the key while you have changes it has not saved yet. Touch your YubiKey to save them; if you cannot, use **Export Vault** to keep a copy before you lock the vault.
