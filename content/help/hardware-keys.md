...
A similar banner appears for a vault kept in a folder rather than on iCloud Drive or Google Drive, when you save it in KeePassXC on the same Mac or it arrives through a sync tool such as Syncthing or Dropbox. There it is PanicVault's own saves that wait for the key, so it says **Changes not saved to the vault file — YubiKey needed.** and **This vault's file was changed by another app with different encryption settings.** Until then your changes are not saved to the file. If you unlocked with Face ID or Touch ID, use **Export Vault** before you lock and unlock with your master password and YubiKey. A key that does not answer says **That key didn't unlock the vault file — check the slot, or the key.** See [Vaults Synced by Another App](/help/getting-started/#vaults-synced-by-another-app).
