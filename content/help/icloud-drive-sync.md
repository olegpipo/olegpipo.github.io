...
- iCloud itself kept two copies of the vault file, because both devices saved it at the same moment: PanicVault merges them on its own. The one exception to keeping both versions: for an entry changed in both copies, PanicVault keeps the newer change (newer by each device's own clock) and puts the other in the entry's [history](/help/entries/#entry-history) -- or, when the history can't hold it, keeps it as a "(conflicted copy)". Such a copy can also appear for an entry changed or moved in only one of the two copies, when its history can't hold the other version. A copy that can't be opened -- saved after the password was changed on another device, for example -- stays unmerged until PanicVault can open it.
